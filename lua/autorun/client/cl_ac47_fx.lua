...
--
-- CreateSound deduplicates per (entity, path) regardless of
-- sound.Add channel field — unusable for stacking.
--
-- Entity:EmitSound(path, level, pitch, vol, channel) with an
-- explicit channel number is the only GMod API that lets you
-- run multiple sounds simultaneously on the same entity.
-- Each gun gets its own channel constant so they never stomp:
--   Gun 1 -> CHAN_WEAPON  (0)
--   Gun 2 -> CHAN_VOICE   (2)
--   Gun 3 -> CHAN_ITEM1   (5)
--
-- To stop a specific channel: Entity:StopSound(path) does NOT
-- target by channel. Correct stop API is
-- Entity:EmitSound("common/null.wav", ..., channel) which
-- immediately cuts the channel by replacing it with silence.
-- ============================================================

local GUN_WAV  = "lfs/tfre_ac47/m134_shoot.wav"
local NULL_WAV = "common/null.wav"

-- One channel constant per gun slot. Must be distinct.
...
net.Receive("ac47_gun_sound", function()
    ...
    if isStart then
        ...
        -- Cut the channel immediately with a null wav
